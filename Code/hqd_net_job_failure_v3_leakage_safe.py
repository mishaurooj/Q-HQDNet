# -*- coding: utf-8 -*-
"""
HQD-Net v3: Leakage-Safe Hierarchical Quantum-Dependency Network
...
python hqd_net_job_failure_v3_leakage_safe.py ^
  ...
  --out-dir "D:\\other\\ALIBABAQUATUM\\Results_HQDNet_v3" ^
...
def sample_stratified(df: pd.DataFrame, n: int) -> pd.DataFrame:
    ...
    parts = []
    for cls, g in df.groupby("job_failure"):
...
def train_single_model(df: pd.DataFrame, features: List[str], model_name: str, sample_size: int, split_mode: str = "iid", scaler: str = "standard"):
    data = sample_stratified(df, sample_size)
...
class HQDNet:
    """Complexity-routed hybrid classifier: scalable classical branch + quantum-inspired branch for complex jobs."""
    def __init__(self, features: List[str], structural_features: List[str], tau_quantile: float = 0.80,
                 classical_name: str = "XGB", quantum_name: str = "MLP"):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series):
        ...
        self.classical_pipe_ = Pipeline([
            ...
            ("clf", make_model(self.classical_name, pos_weight)),
        ])
        self.classical_pipe_.fit(X[self.features], y)
        self.classical_threshold_ = tune_threshold(y, predict_scores(self.classical_pipe_, X[self.features]))
        ...
        if route_mask.sum() < max(30, int(0.03 * len(X))):
        ...
        self.quantum_threshold_ = tune_threshold(y.loc[route_mask], predict_scores(self.quantum_pipe_, X.loc[route_mask, q_features]))
        return self
    ...
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if route_mask.any():
            ...
            score[route_mask] = 0.40 * pc[route_mask] + 0.60 * pq
...
def train_hqdnet(df: pd.DataFrame, features: List[str], groups: Dict[str, List[str]], sample_size: int, tau_quantile: float, split_mode: str = "iid"):
    data = sample_stratified(df, sample_size)
    X_train, y_train, X_test, y_test = split_data(data, features, split_mode)
...
def run_ablations(df: pd.DataFrame, features: List[str], groups: Dict[str, List[str]], csv_dir: Path, fig_dir: Path, model_dir: Path, n: int):
    ...
    for name, fs in feature_sets.items():
        if not fs: continue
        try:
            m, _, _ = train_single_model(df, fs, ab_model, n, split_mode="iid")
    ...
    for tq in [0.50, 0.60, 0.70, 0.80, 0.90, 0.95]:
        try:
            m, model = train_hqdnet(df, features, groups, n, tau_quantile=tq)
    ...
    for split in ["iid", "temporal"]:
        for name in ["RF", ab_model]:
            try:
                m, _, _ = train_single_model(df, features, name, n, split_mode=split)
                m["split"] = split; rows.append(m)
            except Exception as e: log(f"[warn] drift {name}-{split}: {e}")
        try:
            m, _ = train_hqdnet(df, features, groups, n, tau_quantile=0.80, split_mode=split)
            m["split"] = split; rows.append(m)
    ...
    rows = []; base = sample_stratified(df, n)
    for sigma in [0.00, 0.01, 0.03, 0.05, 0.10]:
        ...
        for name in ["RF", ab_model]:
            try:
                m, _, _ = train_single_model(noisy, features, name, min(n, len(noisy)), split_mode="iid")
...
def run_main(df: pd.DataFrame, out_dir: Path, sample_sizes: List[int], run_abl: bool):
    ...
    for n in sample_sizes:
        ...
        for name in base_models:
            try:
                ...
                m, model, _ = train_single_model(df, features, name, n, split_mode="iid")
        ...
        try:
            ...
            m, model = train_hqdnet(df, features, groups, n, tau_quantile=0.80)
    ...
    plot_lines(metrics, fig_dir / "fig_scalability_pr_auc.png", "sample_size", "pr_auc", "model", "Leakage-Safe Scalability Evaluation", "PR-AUC")
    plot_lines(metrics, fig_dir / "fig_scalability_f1.png", "sample_size", "f1", "model", "Leakage-Safe F1 Scalability Evaluation", "F1")
    ...
    for n in sample_sizes:
        plot_metric_bars(metrics, fig_dir / f"fig_model_metrics_n{n}.png", f"Leakage-Safe Predictive Performance at n={n}", sample_size=n)
    if run_abl:
        run_ablations(df, features, groups, csv_dir, fig_dir, model_dir, max(sample_sizes))
    ...
    readme = [
        ...
        "## Key Files", "",
        "- `csv_results/main_metrics_all_sample_sizes.csv`", "- `csv_results/ablation_feature_blocks.csv`", "- `csv_results/ablation_temporal_drift.csv`", "- `csv_results/ablation_routing_threshold.csv`", "- `csv_results/ablation_noise_robustness.csv`", "- `figures/`", "- `trained_models/`"
    ]
...
def parse_args():
    p = argparse.ArgumentParser(description="HQD-Net v3 leakage-safe Alibaba job failure prediction")
...
def main():
    ...
    run_main(df, out_dir, args.sample_sizes, args.run_ablations)
