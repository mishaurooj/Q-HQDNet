...
def write_readme(dirs: Dict[str, Path], metrics: pd.DataFrame, raw_dist: Dict[str, Any]) -> None:
    ...
    lines.append("")
    lines.append("This run reports a full proposed system, not a standalone weak quantum branch. HQD-Net-v7 uses the strongest classical backbone with hard-example structural correction and validation-tuned fusion.")
    lines.append("")
...
def main():
    ...
    df_all[META_COLUMNS + [c for c in groups["all"] if c in df_all.columns]].head(200000).to_parquet(dirs["data"] / "processed_preview.parquet", index=False)
