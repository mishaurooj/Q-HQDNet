# -*- coding: utf-8 -*-
"""
HQD-Net v4: Balanced Leakage-Safe Hard-Example Quantum-Dependency Network
...
def sample_dataset(df: pd.DataFrame, n: int, sampling_mode: str = "balanced") -> pd.DataFrame:
    """
    Sampling policy for experiments.

    balanced: creates an approximately 50/50 failure/success dataset by undersampling
              the majority class and oversampling the minority class if needed. This is
              the default because raw Alibaba job failure labels are highly imbalanced.
    imbalanced: preserves the natural class ratio for deployment realism.
    """
    data = df[df["job_failure"].isin([0, 1])].copy()
    if len(data) == 0:
        raise RuntimeError("No valid labels found in job_failure.")
    if n >= len(data) and sampling_mode != "balanced":
    ...
    if sampling_mode == "balanced":
        ...
        parts = []
        for cls in [0, 1]:
            g = data[data["job_failure"] == cls]
            if len(g) == 0:
                raise RuntimeError(f"Class {cls} is missing; cannot build a balanced dataset.")
            replace = len(g) < half
            parts.append(g.sample(n=half, replace=replace, random_state=RANDOM_STATE + int(cls)))
        out = pd.concat(parts, axis=0).sample(frac=1, random_state=RANDOM_STATE).reset_index(drop=True)
        return out.head(n).copy()

    # natural, imbalanced sampling
    if n >= len(data):
        return data.sample(frac=1, random_state=RANDOM_STATE).copy()
    parts = []
    for cls, g in data.groupby("job_failure"):
        k = max(1, int(round(n * len(g) / len(data))))
        k = min(k, len(g))
        parts.append(g.sample(n=k, random_state=RANDOM_STATE + int(cls)))
    out = pd.concat(parts).sample(frac=1, random_state=RANDOM_STATE).head(n).copy()
    return out


def sample_stratified(df: pd.DataFrame, n: int) -> pd.DataFrame:
    # Backward-compatible alias. New code should call sample_dataset(..., sampling_mode=...).
    return sample_dataset(df, n, sampling_mode="balanced")
...
def split_data(data: pd.DataFrame, features: List[str], split_mode: str):
    ...
    if split_mode == "temporal" and "first_start_time" in data.columns:
        ...
        # If temporal cut degenerates under balanced oversampling, fall back to IID split.
        if train["job_failure"].nunique() < 2 or test["job_failure"].nunique() < 2:
            train, test = train_test_split(data, test_size=0.2, stratify=data["job_failure"], random_state=RANDOM_STATE)
    else:
        train, test = train_test_split(data, test_size=0.2, stratify=data["job_failure"], random_state=RANDOM_STATE)
    return train[features], train["job_failure"].astype(int), test[features], test["job_failure"].astype(int)


def split_train_val_test(data: pd.DataFrame, features: List[str], split_mode: str):
    X_train, y_train, X_test, y_test = split_data(data, features, split_mode)
    tr = X_train.copy(); tr["job_failure"] = y_train.values
    if tr["job_failure"].nunique() < 2 or len(tr) < 20:
        return X_train, y_train, X_train, y_train, X_test, y_test
    tr_main, val = train_test_split(tr, test_size=0.2, stratify=tr["job_failure"], random_state=RANDOM_STATE)
    return (tr_main[features], tr_main["job_failure"].astype(int),
            val[features], val["job_failure"].astype(int),
            X_test, y_test)


def train_single_model(df: pd.DataFrame, features: List[str], model_name: str, sample_size: int, split_mode: str = "iid", scaler: str = "standard", sampling_mode: str = "balanced"):
    ...
    train_scores = predict_scores(pipe, X_train)
    threshold = tune_threshold(y_train, train_scores)
...
def train_hqdnet(df: pd.DataFrame, features: List[str], groups: Dict[str, List[str]], sample_size: int, tau_quantile: float, split_mode: str = "iid", sampling_mode: str = "balanced"):
    data = sample_dataset(df, sample_size, sampling_mode=sampling_mode)
    X_train, y_train, X_test, y_test = split_data(data, features, split_mode)
...
def run_ablations(df: pd.DataFrame, features: List[str], groups: Dict[str, List[str]], csv_dir: Path, fig_dir: Path, model_dir: Path, n: int, sampling_mode: str):
    ...
    for name, fs in feature_sets.items():
        if not fs: continue
        try:
            m, _, _ = train_single_model(df, fs, ab_model, n, split_mode="iid", sampling_mode=sampling_mode)
    ...
    for sigma in [0.00, 0.01, 0.03, 0.05, 0.10]:
        ...
        for name in ["RF", ab_model]:
            try:
                m, _, _ = train_single_model(noisy, features, name, min(n, len(noisy)), split_mode="iid", sampling_mode=sampling_mode)
...
def run_main(df: pd.DataFrame, out_dir: Path, sample_sizes: List[int], run_abl: bool, sampling_mode: str):
    ...
    for n in sample_sizes:
        ...
        for name in base_models:
            try:
                ...
                m, model, _ = train_single_model(df, features, name, n, split_mode="iid", sampling_mode=sampling_mode)
    ...
    plot_lines(metrics, fig_dir / "fig_scalability_pr_auc.png", "sample_size", "pr_auc", "model", "Balanced Leakage-Safe Scalability Evaluation", "PR-AUC")
    plot_lines(metrics, fig_dir / "fig_scalability_f1.png", "sample_size", "f1", "model", "Balanced Leakage-Safe F1 Scalability Evaluation", "F1")
    ...
    for n in sample_sizes:
        plot_metric_bars(metrics, fig_dir / f"fig_model_metrics_n{n}.png", f"Balanced Leakage-Safe Predictive Performance at n={n}", sample_size=n)
    if run_abl:
    ...
    readme = [
        ...
        f"Sampling mode: `{sampling_mode}`. This run removes post-execution leakage features and reports failure-focused metrics.", "",
...
def parse_args():
    ...
    p.add_argument("--sampling-mode", choices=["balanced", "imbalanced"], default="balanced", help="balanced gives 50/50 failure/success samples; imbalanced preserves natural deployment skew.")
    return p.parse_args()
